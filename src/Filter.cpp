...
Result Filter::createMediaFilterEventWithIon(vector<uint8_t> output) {
    ...
    mediaEvent = {
        .streamId = 0,  // Укажите правильное значение
        .isPtsPresent = false,  // Укажите правильное значение
        .pts = mPts,
        .dataLength = static_cast<uint32_t>(output.size()),
        .offset = 0,  // Укажите правильное значение
        .avMemory = std::move(handle),
        .isSecureMemory = false,  // Укажите правильное значение
        .avDataId = dataId,
        .mpuSequenceNumber = 0,  // Укажите правильное значение
        .isPesPrivateData = false,  // Укажите правильное значение
        .extraMetaData = {},  // Укажите правильное значение или инициализируйте по-другому
    };
    if (mPts) {
        mediaEvent.pts = mPts;
        mPts = 0;
